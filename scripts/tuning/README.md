...
| `run_holdout_predict_pcs.R` | Fit on **220 PCs** in `selection_train`, predict **`TOTEXP` for all 2000 test rows** → `holdout_test_predictions*.parquet` + `.json` (default **XGB**; `MODEL=glmnet` ridge; **`MODEL=rf`** / **`MODEL=lgb`** → **`holdout_test_predictions_rf`** / **`holdout_test_predictions_lgb`**) |
