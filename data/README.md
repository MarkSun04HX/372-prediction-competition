...
| `processed/selection_data.parquet` | **Selection data:** **10,000** random rows from the pooled file, **`PC1`–`PC220`** from PCA (`irlba`) fit on that sample’s numeric predictors, plus **`TOTEXP`** and **`FYC_YEAR`**. Built by `scripts/build_selection_data.R` (gitignored like other `.parquet`). |
...
```bash
Rscript scripts/build_selection_data.R
# Optional: SEED=42 N_ROW=10000 N_PC=220 Rscript scripts/build_selection_data.R
```
...
```bash
Rscript scripts/run_lasso_elasticnet_selection.R
```
